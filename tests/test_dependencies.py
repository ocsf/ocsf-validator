...
from ocsf_validator.reader import Reader, DictReader
...
def test_profiles():
    ...
    assert "thing" in r["/extensions/one/events/network/http_activity.json"]["attributes"]
    assert "thing2" not in r["/extensions/one/events/network/http_activity.json"]["attributes"]
